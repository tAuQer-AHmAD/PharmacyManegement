        ...
        <div class="item" id="main">
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>
                        <button class="btn" style="border-radius: 10px;" type="submit"><a class="btn btn-primary"
                                href="{{URL::to('sale/update', $sale->id)}}" title="Edit -> {{$sale->name}}">
                                Update
                            </a></button>
                    </td>
                    <td>
                        <a href="{{ route('sale.read') }}" onclick="event.preventDefault();
                    document.getElementById(
                      'delete-form-{{$sale->id}}').submit();">
                            Delete
                        </a>
                    </td>

                    <form id="delete-form-{{$sale->id}}" + action="{{route('sale.destroy', $sale->id)}}" method="post">
                        @csrf @method('DELETE')
                    </form>
